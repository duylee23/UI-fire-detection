...
<div className="h-screen bg-gradient-to-br from-slate-100 via-slate-200 to-slate-300 text-slate-800 flex overflow-hidden">
      ...
      <div className="w-80 bg-white/80 backdrop-blur-sm border-r border-blue-200 flex flex-col shadow-lg">
        ...
        <div className="px-6 pb-6 border-b border-blue-200">
          <button
            className="flex items-center justify-between w-full gap-2 mb-4 select-none focus:outline-none"
          ...
          >
            ...
            {settingsOpen ? <ChevronDown className="w-4 h-4 text-slate-500" /> : <ChevronRight className="w-4 h-4 text-slate-500" />}
          </button>
          {settingsOpen && (
        ...
        <div className="px-6 pb-6 border-b border-blue-200">
          <button
            className="flex items-center justify-between w-full gap-2 mb-4 mt-2 select-none focus:outline-none"
            onClick={() => setCameraConfigOpen((v) => !v)}
          ...
          >
            ...
            {cameraConfigOpen ? <ChevronDown className="w-4 h-4 text-slate-500" /> : <ChevronRight className="w-4 h-4 text-slate-500" />}
          </button>
          {cameraConfigOpen && (
            <>
            ...
            <div className="space-y-2">
              {cameras.map((camera) => (
                <button
                  ...
                  className={`w-full text-left p-3 rounded-lg transition-all flex items-center justify-between relative
                    ${selectedCamera === camera.id ? 'bg-blue-100 border border-blue-300 text-blue-700' : 'bg-slate-100 hover:bg-slate-200 text-slate-700'}
                    ${camera.hasAlert ? 'animate-blink-red border-2 border-red-500 shadow-lg' : ''}
                  `}
                  style={camera.hasAlert ? { animationDuration: '1s' } : {}}
                >
                  <div className="flex items-center gap-3">
                    ...
                    {camera.hasAlert && (
                      <span className="ml-2 flex items-center">
                        <span className="w-3 h-3 bg-red-500 rounded-full animate-pulse mr-1"></span>
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-4">
              ...
              {selectedCameraData?.hasAlert && (
                <div className="flex items-center gap-2 px-3 py-1 bg-red-500/20 border border-red-500/50 rounded-full animate-blink-red">
                  <AlertTriangle className="w-4 h-4 text-red-500 animate-pulse" />
                  <span className="text-sm text-red-600 font-bold animate-pulse">FIRE/SMOKE DETECTED</span>
                </div>
              )}
            </div>
